...
namespace eosio
{
                          ...
                          const eosio::binary_extension<eosio::checksum256> &proposal_hash)
   {
      ...
      {
         auto itr = std::find_if(apps_it->requested_approvals.begin(), apps_it->requested_approvals.end(), [&](const approval &a)
   ...
   void multisig::unapprove(name proposer, name proposal_name, permission_level level)
   {
      ...
      if (apps_it != apptable.end())
      {
         auto itr = std::find_if(apps_it->provided_approvals.begin(), apps_it->provided_approvals.end(), [&](const approval &a)
                                 { return a.level == level; });
   ...
   void multisig::exec(name proposer, name proposal_name, name executer)
   {
      ...
      auto table_op = [](auto &&table, auto &&table_iter)
